...
__device__ void
genTransposeConvIjkForGridCallback(int32_t bidx,
                                   int32_t lidx,
                                   int32_t vidx,
                                   int32_t cidx,
...
JaggedTensor
genTransposeConvIJKForGrid(const GridBatchImpl &batchHdl,
                           nanovdb::Coord upsamplingFactor,
    ...
    auto cb = [=] __device__(int32_t bidx,
        ...
        genTransposeConvIjkForGridCallback(
    ...
    if (maybeMask.has_value()) {
        torch::Tensor mask = maybeMask.value().jdata().view({-1});
...
dispatchBuildGridForGenTransposeConv<torch::kCUDA>(const GridBatchImpl &coarseBatchHdl,
                                                   const nanovdb::Coord &stride,
                                                   const std::optional<JaggedTensor> &subdivMask) {
    JaggedTensor coords = genTransposeConvIJKForGrid(coarseBatchHdl, stride, subdivMask);
    return ops::dispatchCreateNanoGridFromIJK<torch::kCUDA>(coords);
}

template <>
nanovdb::GridHandle<TorchDeviceBuffer>
dispatchBuildGridForGenTransposeConv<torch::kCPU>(const GridBatchImpl &coarseBatchHdl,
        ...
        for (auto it = ActiveVoxelIterator<-1>(coarseTree); it.isValid(); it++) {
            ...
            for (int i = 0; i < subdivisionFactor[0]; i += 1) {
                for (int j = 0; j < subdivisionFactor[1]; j += 1) {
                    for (int k = 0; k < subdivisionFactor[2]; k += 1, h += 1) {
                        if (subdivMaskAcc.size(0) > 0 && !subdivMaskAcc[it->second + joffset + h]) {
                            continue;
                        }
